...
object App {
  ...
  def main(args:Array[String]): Unit = {
    ...
    // TODO uncomment each section as you go
...
//    // option / try
//    val str = "a1.b2.c3.d4"
//    println(getNth(str, 3))
//
//    println(tryGetNth(str, 3))
//    tryGetNth(str, 5) match {
//      case Success(i) => println(i)
//      case Failure(e) => println(s"Failed with $e")
//    }
//
//    var result = optionGetNth(str, 4) // returns d4
//      .map(entry => entry.toUpperCase()) // returns D4
//      .filter(entry => entry.contains('4')) // returns D4
//      .map(entry => entry + '$') // returns D4$
//      .map(entry => entry.map(e => e.equals('D'))) // returns true false false
//      .getOrElse("failed")
//
//    result = tryGetNth(str, 4) // returns d4
//      .map(entry => entry.toUpperCase()) // returns D4
//      .filter(entry => entry.contains('4')) // returns D4
//      .map(entry => entry + '$') // returns D4$
//      .map(entry => entry.map(e => e.equals('D'))) // returns true false false
...
//    // note could read as json, but this will give us some practice
    // todo load dataframe from file and show contents
...
//    for(i <- 1 to 5){ // for loop is cheating. scala is not purely functional
...
//
//    // todo extraction for other columns
//    newDf = df
...
//  val getColumnContentsUdf: UserDefinedFunction = udf(parseContents _)
  ...
  val lowerRemoveWhiteUdf = udf[Option[String], String](betterLowerRemoveAllWhitespace)
